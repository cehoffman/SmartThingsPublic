	...
	if (cmd.commandClass == 50) { //50 is likely a manufacturer specific code, Z-Wave specifies this as a "Basic Window Covering" so it's not a true Z-Wave Command Class.   
        ...
        if (encapsulatedCommand) {
        	...
        	switch (cmd.sourceEndPoint) {
            	case 1:
                case 2:
  					lead = cmd.sourceEndPoint
                    break
                default:
                	log.debug "Unknown source ${cmd.sourceEndPoint} ${encapsulatedCommand}"
                	return
            }
            ...
            switch (encapsulatedCommand.scale) {
            	case 0: // kWh
                	name = "consumption"
                    value = Math.round(encapsulatedCommand.scaledMeterValue)
                    description = "${value} kWh"
                	break;
            	case 2: // Watts
                	name = "power"
                    value = Math.round(encapsulatedCommand.scaledMeterValue)
                    description = "${value} Watts"
                	break
                case 4: // Volts
                	name = "volts"
                    value = Math.round(encapsulatedCommand.scaledMeterValue * 100) / 100
                    description = "${value} Volts"
                    break
                case 5: // Amps
                	name = "current"
                    value = Math.round(encapsulatedCommand.scaledMeterValue * 100) / 100
                    description = "${value} Amps"
                	break
                default:
                	name = "unknown"
                    value = encapsulatedCommand.scale
                	description = "Unknown: ${encapsulatedCommand.scale} ${String.format("%5.2f", encapsulatedCommand.scaledMeterValue)}"
                	// unknown
            }
            
            sendEvent(name: "${name}${lead}", value: value as String, unit: "", descriptionText: "L${lead}: ${description}")
		}
	}
}
