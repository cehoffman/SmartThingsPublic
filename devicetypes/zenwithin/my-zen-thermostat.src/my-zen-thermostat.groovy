 ...
 */
metadata {
  definition (name: "My Zen Thermostat", namespace: "zenwithin", author: "ZenWithin") {
    capability "Actuator"
  ...
  tiles {
        ...
        standardTile("fanMode", "device.thermostatFanMode", decoration: "flat") {
            ...
            state "fanOn", action:"thermostat.setThermostatFanMode", backgroundColor:"#ffffff", icon:"st.thermostat.fan-on"
        }
        ...
        standardTile("mode", "device.thermostatMode", decoration: "flat") {
            ...
            state "cool", action:"setThermostatMode", backgroundColor:"#90d0e8", icon:"st.thermostat.cool", nextState:"auto"
        ...
        standardTile("thermostatOperatingState", "device.thermostatOperatingState", inactiveLabel: false) {
            ...
            state "cooling", backgroundColor:"#90d0e8", icon:"st.thermostat.cool"
            state "fan only", backgroundColor:"#e8e3d8", icon:"st.thermostat.fan-on"
            state "idle", backgroundColor:"#e8e3d8", icon:"st.thermostat.heating-cooling-off"
		}
...
def parse(String description) {
    ...
    if (description?.startsWith("read attr -")) 
    {
        ...
        if (descMap.cluster == "0201" && descMap.attrId == "0000") 
        {
            ...
            map.value = getTemperature(descMap.value, 1)
...
def getTemperature(value, precision = 0) 
{
  ...
  if(getTemperatureScale() == "F"){
    returnValue = decimalFormat.format(Math.round(celsiusToFahrenheit(celsius)*(10**precision))/(10.0**precision))
...
def setSetpoint(degrees) 
{
    ...
    def degreesDouble = (degrees as Double).round()
